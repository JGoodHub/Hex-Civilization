...
public class TileController : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        for (int y = 0; y < boardSize.y; y++)
        {
            for (int x = 0; x < boardSize.x; x++)
            {
                ...
                int distToEdge = Mathf.Min(x, boardSize.x - x, y, boardSize.y - y);
                if (distToEdge < edgeFalloff)
                    sample *= (float)distToEdge / edgeFalloff;
    ...
    {
        return new Vector2((position.x * BiomeTileData.SIZE.x) + (position.y % 2 * (BiomeTileData.SIZE.x / 2)), position.y * (BiomeTileData.SIZE.y * 0.75f));
    }
        ...
        if (drawLatitudeBoundaries)
        {
            for (int y = 0; y < boardSize.y; y++)
            {
                ...
                for (int x = 0; x < boardSize.x; x++)
                {
                    ...
                    Gizmos.DrawCube(GridToWorldPosition(new Vector2Int(x, y)), BiomeTileData.SIZE);
                }
            }
        }
    }
